...
public class CrazyLambdas {
    ...
    public static Predicate<String> lengthInRangePredicate(int min, int max) {
        return x -> x.length() < max && x.length() >= min;
    }
    ...
    public static IntSupplier randomIntSupplier() {
        return ()->{ Random rand = new Random(); return rand.nextInt();};
    }
    ...
    public static Supplier<Thread> runningThreadSupplier(Runnable runnable) {
        return () -> { Thread thread = new Thread(runnable); thread.start(); return thread; };
    }
    ...
    public static Consumer<Runnable> newThreadRunnableConsumer() {
        return x -> { Thread thread = new Thread(x); thread.run(); };
    }
    ...
    public static Function<Runnable, Supplier<Thread>> runnableToThreadSupplierFunction() {
        return x -> ()-> { Thread thread = new Thread(x); thread.run(); return thread; };
    }
    ...
    public static BiFunction<IntUnaryOperator, IntPredicate, IntUnaryOperator> functionToConditionalFunction() {
        return (x, y) -> { return z -> { if(y.test(z)) { return x.applyAsInt(z); } else { return z; } }; };
    }
    ...
    public static Supplier<Supplier<Supplier<String>>> trickyWellDoneSupplier() {
        return ()-> { return ()->{ return ()-> { return "WELL DONE!"; }; }; };
    }
}
